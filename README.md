...
# שלב ד' – מימוש לוגיקה עסקית ותכנות בסיס נתונים (PL/pgSQL)

בשלב זה הורחב בסיס הנתונים ממודל נתונים פסיבי למערכת עסקית אוטונומית ודינמית. המימוש בוצע במלואו בסביבת **PostgreSQL 18** באמצעות שפת **PL/pgSQL**, תוך הטמעת מנגנוני בקרת זרימה, לולאות וסמנים (`Cursors`), טרנזקציות מורכבות, טיפול מובנה בחריגות (`EXCEPTION`), וטריגרים לאכיפת שלמות נתונים ואוטומציה.

---

## תוכן עניינים
1. [סקירת ארכיטקטורת המערכת והסביבה](#סקירת-ארכיטקטורת-המערכת-והסביבה)
2. [פונקציות (Stored Functions)](#1-פונקציות-stored-functions)
   * [פונקציה 1: חישוב שווי שוק משוקלל לשחקן](#פונקציה-1-חישוב-שווי-שוק-משוקלל-לשחקן-fn_calculate_player_market_value)
   * [פונקציה 2: בדיקת תאימות חוקי סגל וחישוב שווי](#פונקציה-2-בדיקת-תאימות-חוקי-סגל-וחישוב-שווי-fn_evaluate_squad_compliance)
3. [פרוצדורות (Stored Procedures)](#2-פרוצדורות-stored-procedures)
   * [פרוצדורה 1: ניהול עסקאות והעברות שחקנים](#פרוצדורה-1-ניהול-עסקאות-והעברות-שחקנים-sp_process_player_transfer)
   * [פרוצדורה 2: יישוב וסגירת מחזור משחקים](#פרוצדורה-2-יישוב-וסגירת-מחזור-משחקים-sp_settle_round)
4. [טריגרים (Triggers)](#3-טריגרים-triggers)
   * [טריגר 1: חישוב מחיר שחקן אוטומטי בעת שינוי ציונים](#טריגר-1-חישוב-מחיר-שחקן-אוטומטי-בעת-שינוי-ציונים-trg_students_price_autocalc)
   * [טריגר 2: יומן ביקורת תקציב וחסימת יתרת חובה](#טריגר-2-יומן-ביקורת-תקציב-וחסימת-יתרת-חובה-trg_users_budget_audit)
5. [סיכום ואימות מערכתי](#4-סיכום-ואימות-מערכתי)

---

## סקירת ארכיטקטורת המערכת והסביבה

* **מנוע בסיס הנתונים:** PostgreSQL 18.
* **סכימת נתונים:** 17 טבלאות ליבה ואינטגרציה המכילות מעל 76,000 רשומות (שחקנים, משחקים, אירועים, משתמשים, סגלים, טרנזקציות ומחזורים).
* **מיקום קובצי המקור:** תיקיית `step4/`.

---

## 1. פונקציות (Stored Functions)

### פונקציה 1: חישוב שווי שוק משוקלל לשחקן (`fn_calculate_player_market_value`)
* **קובץ מקור:** `step4/function1.sql`
* **מטרה עסקית:** חישוב דינמי של שווי שוק עבור שחקן (`student_id`) על סמך שקלול ביצועיו במשחקים בפועל: שערים (+5%), בישולים (+3%), כמות הופעות (+1%), כרטיסים צהובים (-1.5%) וכרטיסים אדומים (-4%).
* **רכיבים תכנותיים:** לולאת `FOR ... IN (SELECT ...)` לאיטרציה על אירועי המשחק, תנאי `CASE`, חישוב אלגוריתמי מעוגל, ובלוק `EXCEPTION` לטיפול בשגיאות קלט.
* **תרחיש בדיקה:** קריאה לפונקציה עבור שחקן `100000001` מציגה חישוב שווי מדויק והדפסת הודעת `NOTICE`.

![הרצת פונקציה 1](images/function1_execution.png)

---

### פונקציה 2: בדיקת תאימות חוקי סגל וחישוב שווי (`fn_evaluate_squad_compliance`)
* **קובץ מקור:** `step4/function2.sql`
* **מטרה עסקית:** אימות עמידת סגל המשתמש בכללי ליגת הפנטזי (הרכב פותח של 11 שחקנים, חלוקת ספסל, שווי סגל מצטבר ויתרת תקציב) תוך החזרת אובייקט תוצאה מובנה.
* **רכיבים תכנותיים:** הגדרת טיפוס מותאם אישית (`TYPE squad_compliance_result`), שאילתות אגרגציה, תנאים לוגיים מורכבים, ובדיקת קיום משתמש עם זריקת חריגה מבוקרת.
* **תרחישי בדיקה:**
  1. **אימות סגל קיים:** בדיקת משתמש `1` – זיהוי חריגת חוקים מדויקת (10 שחקנים פותחים במקום 11) והחזרת `is_compliant = false`.
  2. **בדיקת שגיאה (Negative Test):** קריאה למשתמש לא קיים (`999999`) נתפסת ב-`EXCEPTION` ומחזירה הודעה מתאימה.

![הרצת פונקציה 2](images/function2_execution.png)
...
* **מטרה עסקית:** ניהול עסקאות קנייה (`BUY`) ומכירה (`SELL`) של שחקנים, כולל בדיקת יתרת תקציב, ניכוי עמלת שוק (5% במכירה), עדכון טבלת הסגל (`User_Squads`), עדכון תקציב המשתמש, ותיעוד ב-`Transactions`.
* **רכיבים תכנותיים:** פרמטרי `IN/OUT`, בקרת כפילויות, עדכון מספר טבלאות בתוך יחידה טרנזקציונית אחת, והחזרת סטטוס הצלחה והודעה מפורטת.
* **תרחישי בדיקה:**
  1. **קנייה תקינה:** רכישת שחקן `100000005` על ידי משתמש `1` ועדכון תקציב.
  2. **מניעת כפילות:** ניסיון רכישה חוזר של אותו שחקן נחסם (`p_success = false`).
  3. **מכירה וניכוי עמלה:** מכירת השחקן, זיכוי המשתמש ב-95% מערך השחקן, ואימות יצירת רשומות בטבלת `Transactions`.

![הרצת פרוצדורה 1](images/procedure1_execution.png)

---

### פרוצדורה 2: יישוב וסגירת מחזור משחקים (`sp_settle_round`)
* **קובץ מקור:** `step4/procedure2.sql`
* **מטרה עסקית:** ביצוע סגירה תקופתית של מחזור משחקים פעיל: עיבוד אלפי שחקנים, חישוב שווי השוק החדש שלהם בעזרת `fn_calculate_player_market_value`, שמירת רשומות היסטוריות ב-`Price_History`, עדכון סטטוס המחזור ל-`Completed`, והפעלת המחזור הבא.
* **רכיבים תכנותיים:** לולאות סמן מרובות נתונים, מוני בקרה (`p_players_processed`, `p_errors_encountered`), חסימת הרצה על מחזור שאינו `Active`, וסנכרון תלויות בין מחזורים.
* **תרחישי בדיקה:**
  1. **סגירת מחזור תקינה:** עיבוד 4,499 שחקנים במחזור `503`, עדכון מחיריהם והפיכת המחזור ל-`Completed`.
  2. **בדיקת נעילה:** ניסיון להריץ שוב סגירה על מחזור שכבר הושלם נחסם מיד בשגיאת `ERROR: Round cannot be settled`.

![הרצת פרוצדורה 2](images/procedure2_execution.png)

---

## 3. טריגרים (Triggers)

### טריגר 1: חישוב מחיר שחקן אוטומטי בעת שינוי ציונים (`trg_students_price_autocalc`)
* **קובץ מקור:** `step4/trigger1.sql`
* **מטרה עסקית:** אוטומציה מוחלטת של מחירי השחקנים: בכל פעם שנוצר שחקן חדש או מתעדכנים ציוני היכולת שלו (`technical_rating`, `mental_rating`), הטריגר מחשב ומעדכן את `current_price` לפני שמירת השורה.
* **סוג טריגר:** `BEFORE INSERT OR UPDATE ON Students FOR EACH ROW`.
* **תרחישי בדיקה:**
  1. הכנסת שחקן חדש עם ציונים 85 ו-70 – הטריגר מחשב אוטומטית שווי של 110.50.
  2. עדכון הציון הטכני ל-95 – הטריגר מעדכן מיד את השווי ל-117.50 ללא צורך בהתערבות ידנית.

![הרצת טריגר 1](images/trigger1_execution.png)

---

### טריגר 2: יומן ביקורת תקציב וחסימת יתרת חובה (`trg_users_budget_audit`)
* **קובץ מקור:** `step4/trigger2.sql`
* **מטרה עסקית:** 
  1. **שמירת שלמות נתונים:** חסימה מוחלטת של עדכון תקציב לערך שלילי ברמת מנוע ה-DB.
  2. **Audit Logging:** תיעוד מלא של כל שינוי תקציבי (ערך קודם, ערך חדש, הפרש וזמן מדויק) בטבלת ביקורת ייעודית (`user_budget_audit_log`).
* **סוג טריגר:** `BEFORE UPDATE ON Users FOR EACH ROW`.
* **תרחישי בדיקה:**
  1. **שינוי חוקי:** הפחתת תקציב ב-50.00 יצרה רשומת ביקורת מדויקת בטבלת ה-Audit.
  2. **חסימת חריגה:** ניסיון עדכון תקציב למינוס (`-100.00`) נדחה ונחסם עם שגיאת `ERROR: Update rejected: User budget cannot go negative`.

![הרצת טריגר 2](images/trigger2_execution.png)

---

## 4. סיכום ואימות מערכתי

כלל הרכיבים בשלב ד' נבדקו ואומתו בהצלחה:

| רכיב | שם האובייקט | ייעוד מרכזי | סטטוס בדיקה |
| :--- | :--- | :--- | :--- |
| **Function 1** | `fn_calculate_player_market_value` | חישוב שווי שוק מבוסס סטטיסטיקת אירועים | עבר בהצלחה |
| **Function 2** | `fn_evaluate_squad_compliance` | בדיקת חוקיות הרכב ושווי כולל | עבר בהצלחה |
| **Procedure 1** | `sp_process_player_transfer` | ניהול מסחר והעברות שחקנים אטומי | עבר בהצלחה |
| **Procedure 2** | `sp_settle_round` | יישוב מחזור המוני ועדכון היסטוריה | עבר בהצלחה |
| **Trigger 1** | `trg_students_price_autocalc` | אוטומציית מחירי שחקנים לפי ציונים | עבר בהצלחה |
| **Trigger 2** | `trg_users_budget_audit` | אכיפת יתרה אי-שלילית ויומן שינויים | עבר בהצלחה |