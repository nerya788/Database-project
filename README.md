...
## שלב ג': אינטגרציה ומבטים אנליטיים (Stage C)

### 1. קבלת מערכת שותפה וניתוח הנדוס לאחור (Reverse Engineering)

בשלב זה קיבלנו את מערכת המסחר בשחקנים הכוללת נתוני משתמשים (סוחרים), מחזורים, סגלי שחקנים שנרכשו, עסקאות מסחר והיסטוריית מחירים.

#### אלגוריתם הינדוס לאחור (סכמה לוגית $\leftarrow$ תרשים ERD)
כדי לשחזר את מודל הישויות-קשרים (ERD) מתוך הסכמה הטבלאית (DSD/DDL) של המערכת המקבילה, פעלנו לפי האלגוריתם הבא:

1. **זיהוי ישויות חזקות (Strong Entities):**
   * כל טבלה שבה המפתח הראשי ($PK$) עצמאי ואינו מורכב ממפתחות זרים מוגדרת כישות עצמאית.
   * העמודות שאינן מפתחות זרים ממופות לתכונות (Attributes) של הישות.
   * *דוגמה:* טבלת `USERS` ($PK$: `user_id`), טבלת `ROUNDS` ($PK$: `round_id`).
2. **זיהוי קשרים בינאריים ($1:N$ ו-$1:1$):**
   * טבלה המכילה מפתח זר ($FK$) המצביע לטבלה אחרת מייצגת קשר של יחיד-לרבים ($1:N$). צד ה-$1$ הוא הטבלה אליה מצביעים, וצד ה-$N$ הוא הטבלה המכילה את ה-$FK$.
   * *דוגמה:* המפתח הזר `user_id` בטבלת `TRANSACTIONS` מגדיר קשר $1:N$ בין `USERS` ל-`TRANSACTIONS`.
3. **זיהוי קשרי רבים-לרבים ($M:N$) וישויות קשר (Associative Entities):**
   * טבלה שמפתחה מורכב ממספר מפתחות זרים ומטרתה לקשר בין שתי ישויות חזקות מייצגת קשר של $M:N$.
   * *דוגמה:* טבלת `USER_SQUADS` מקשרת בין סוחר (`user_id`) לתלמיד-שחקן (`player_id`) ומייצגת קשר $M:N$ עם תכונות קשר נוספות (`is_starter`, `purchase_price`).
4. **זיהוי ישויות חלשות / תלויות:**
   * טבלה שהקיום והמפתח שלה תלויים לחלוטין בישות אב (למשל `PRICE_HISTORY` הנשענת על זיהוי שחקן ומחזור).

---

### 2. מתודולוגיית אינטגרציה ובחירת ארכיטקטורה

מתוך שלוש החלופות האפשריות, נבחרה **שיטה א' – אינטגרציה מלאה (Entity Unification & Schema Integration)**.

* **רציונל הבחירה:** 
  מערכת המסחר נשענת על תלמידים אמיתיים שמשחקים בטורניר בתי הספר. במקום להחזיק כפילות של טבלת שחקנים במערכת הסחר לצד טבלת `Students` הקיימת אצלנו, איחדנו את הישויות: כל תלמיד-שחקן בליגת בתי הספר מהווה נכס סחיר במערכת של השותפים.
* **פעולות הסכמה שבוצעו:**
  1. **הרחבת טבלת הליבה `Students`:** הוספת עמודת `current_price` המשקפת את שווי השוק העדכני של התלמיד עבור הסוחרים.
  2. **יצירת טבלת `Team_Players`:** יצירת טבלת גישור המקצה 18 שחקנים לכל אחת מ-500 קבוצות בתי הספר (סה"כ 9,000 שיוכים), כדי לנהל סגל שחקנים מלא לכל בית ספר.
  3. **הטמעת טבלאות המסחר:** יצירת הטבלאות `USERS`, `ROUNDS`, `USER_SQUADS`, `TRANSACTIONS`, ו-`PRICE_HISTORY` עם התאמת מפתחות זרים (`VARCHAR(10)`) המצביעים ישירות ל-`Students.student_id`.
  4. **אכלוס נתונים עקבי (Data Seeding):** שימוש בלוגיקה דטרמיניסטית לגזירת מחירי התלמידים (לפי ציונים טכניים ומנטליים), יצירת 4,500 שיוכי סגלים למשתמשים הסוחרים (15 תלמידים לכל סוחר), והפקת עסקאות והיסטוריית מחירים התואמות במדויק למזהי התלמידים הקיימים במערכת שלנו.

---

<div dir="rtl">

### 3. מבטים אנליטיים ושאילתות עסקיות (Views & Queries)

נוצרו שני מבטים מורכבים המשקפים אינטגרציה מלאה בין מערכת משחקי בתי הספר (התלמידים) למערכת המסחר של השותפים (הסוחרים):

---

#### מבט 1: `v_school_player_market_performance` (נקודת המבט של התלמידים ובתי הספר)

**מהות המבט:**
אינטגרציה המציגה את התלמידים השחקנים על המגרש. המבט משלב את פרטי התלמיד, בית הספר אליו הוא שייך, והביצועים הפיזיים שלו בטורניר (כמו סך השערים שהבקיע), יחד עם נתוני המסחר שלו – שווי השוק הנוכחי שלו וכמה משתמשים סוחרים בחרו לקנות אותו לסגל שלהם בעקבות ביצועיו בטורניר.

**שליפת נתונים מדגמית מהמבט (`SELECT *`):**

שאילתת השליפה:
![View 1 Select Query](images/view_1_select_query.png)

תוצאת השליפה:
![View 1 Sample Result](images/view_1_sample.png)

**שאילתות ניתוח עסקי על מבט 1:**

**שאילתה 1.1:** עשרת התלמידים המבקיעים המובילים בטורניר בתי הספר, לצד שווי השוק שלהם במערכת המסחר.

קוד השאילתה:
![Query 1.1 Code](images/query_1_1_code.png)

תוצאת השאילתה:
![Query 1.1 Result](images/query_1_1_result.png)

**שאילתה 1.2:** איתור תלמידים שחקנים בעלי יכולות טכניות ומנטליות גבוהות אך עם יחס שווי שוק נמוך (הזדמנויות רכישה משתלמות לסוחרים).

קוד השאילתה:
![Query 1.2 Code](images/query_1_2_code.png)

תוצאת השאילתה:
![Query 1.2 Result](images/query_1_2_result.png)

---

#### מבט 2: `v_fantasy_user_portfolio_summary` (נקודת המבט של המשתמשים הסוחרים)

**מהות המבט:**
אינטגרציה המנתחת את מערכת המסחר עצמה. המבט מסכם עבור כל משתמש (סוחר) את יתרת התקציב שלו, השווי הכולל של סגל התלמידים שרכש, עד כמה הוא מגוון את ההשקעה שלו (כמות בתי הספר השונים מהם קנה שחקנים), ואת היקף פעולות המסחר שביצע.

**שליפת נתונים מדגמית מהמבט (`SELECT *`):**

שאילתת השליפה:
![View 2 Select Query](images/view_2_select_query.png)

תוצאת השליפה:
![View 2 Sample Result](images/view_2_sample.png)

**שאילתות ניתוח עסקי על מבט 2:**

**שאילתה 2.1:** דירוג הסוחרים המובילים לפי השווי הכולל של התיק שלהם (תקציב פנוי + שווי התלמידים שרכשו) ורמת הגיוון של בתי הספר מהם הרכיבו את הסגל.

קוד השאילתה:
![Query 2.1 Code](images/query_2_1_code.png)

תוצאת השאילתה:
![Query 2.1 Result](images/query_2_1_result.png)

**שאילתה 2.2:** ניתוח היקף הפעילות בשוק (כמות הטרנזקציות שביצעו) של המשתמשים ביחס ליתרת התקציב הפנויה שלהם.

קוד השאילתה:
![Query 2.2 Code](images/query_2_2_code.png)

תוצאת השאילתה:
![Query 2.2 Result](images/query_2_2_result.png)

---

### דיאגרמות ותרשימי מערכת (Stage C)

#### 1. מערכת השותפים (מערכת המסחר בפני עצמה - Reverse Engineering)

**תרשים ERD של מערכת המסחר:**
![Partner ERD](images/partner_erd.png)

**סכמה לוגית DSD של מערכת המסחר:**
![Partner DSD](images/partner_dsd.png)

---

#### 2. המערכת המאוחדת (School Football & Trading System)

**תרשים ישויות-קשרים מאוחד (Integrated Conceptual ERD):**
![Integrated ERD](images/integrated_erd.png)

**סכמה לוגית מאוחדת (Integrated Physical DSD):**
![Integrated DSD](images/integrated_dsd.png)
...
### 4. תרשימי ERD ואימות נתונים

**קובצי הפרויקט:**
* סקריפט האינטגרציה ואכלוס הנתונים: `step3/Integrate.sql`
* יצירת המבטים והרצת שאילתות הניתוח: `step3/Views.sql`

**אימות כמויות רשומות (Sanity Check):**
* טבלת `Team_Players`: סה"כ 9,000 רשומות.
* טבלת `USERS`: סה"כ 500 משתמשים סוחרים.
* טבלת `ROUNDS`: סה"כ 504 מחזורים.
* טבלת `USER_SQUADS`: סה"כ 4,500 שיוכים (300 סוחרים $\times$ 15 תלמידים שחקנים).
* טבלת `PRICE_HISTORY`: סה"כ 42,550 רשומות מחיר לאורך המחזורים.
* טבלת `TRANSACTIONS`: סה"כ 5,786 עסקאות מסחר.
...
**תרשים ERD של מערכת המסחר (ERD ששוחזר מהסכמה):**
![Partner ERD](images/partner_erd.png)
...
# שלב ד' – תכנות בסיס נתונים ב-PL/pgSQL

בשלב זה הורחבה מערכת בסיס הנתונים `school_football_db` מסכמת אחסון סטטית למערכת תפעולית אוטונומית. כל הלוגיקה העסקית, הטרנזקציות המורכבות, מנגנוני הניטור, האכיפה האוטומטית והתוכניות הראשיות מומשו ברמת מנוע הנתונים באמצעות שפת **PL/pgSQL** בסביבת **PostgreSQL 18**.

---

## תוכן עניינים
1. [פונקציות (Functions)](#1-פונקציות-functions)
   * [פונקציה 1: חישוב שווי שוק משוקלל לשחקן (`fn_calculate_player_market_value`)](#פונקציה-1-חישוב-שווי-שוק-משוקלל-לשחקן-fn_calculate_player_market_value)
   * [פונקציה 2: בדיקת תאימות חוקי סגל ושווי כולל (`fn_evaluate_squad_compliance`)](#פונקציה-2-בדיקת-תאימות-חוקי-סגל-ושווי-כולל-fn_evaluate_squad_compliance)
2. [פרוצדורות (Stored Procedures)](#2-פרוצדורות-stored-procedures)
   * [פרוצדורה 1: ניהול עסקאות והעברות שחקנים (`sp_process_player_transfer`)](#פרוצדורה-1-ניהול-עסקאות-והעברות-שחקנים-sp_process_player_transfer)
   * [פרוצדורה 2: יישוב וסגירת מחזור משחקים (`sp_settle_round`)](#פרוצדורה-2-יישוב-וסגירת-מחזור-משחקים-sp_settle_round)
3. [טריגרים (Triggers)](#3-טריגרים-triggers)
   * [טריגר 1: חישוב אוטומטי של מחיר שחקן בעת שינוי ציונים (`trg_students_price_autocalc`)](#טריגר-1-חישוב-אוטומטי-של-מחיר-שחקן-בעת-שינוי-ציונים-trg_students_price_autocalc)
   * [טריגר 2: יומן ביקורת תקציב ומניעת יתרת חובה (`trg_users_budget_audit`)](#טריגר-2-יומן-ביקורת-תקציב-ומניעת-יתרת-חובה-trg_users_budget_audit)
4. [תוכניות ראשיות (Main Programs)](#4-תוכניות-ראשיות-main-programs)
   * [תוכנית ראשית 1: שקלול מחירי שחקנים וביצוע רכש אוטומטי (`Main1.sql`)](#תוכנית-ראשית-1-שקלול-מחירי-שחקנים-וביצוע-רכש-אוטומטי-main1sql)
   * [תוכנית ראשית 2: בקרת תאימות סגלים ויישוב מחזור ליגה (`Main2.sql`)](#תוכנית-ראשית-2-בקרת-תאימות-סגלים-ויישוב-מחזור-ליגה-main2sql)
5. [טבלת סיכום רכיבי שלב ד'](#5-טבלת-סיכום-רכיבי-שלב-ד)

---

## 1. פונקציות (Functions)

### פונקציה 1: חישוב שווי שוק משוקלל לשחקן (`fn_calculate_player_market_value`)
* **קובץ מקור:** `step4/function1.sql` 
* **מטרה עסקית:** חישוב דינמי של שווי שוק מעודכן לשחקן בהתבסס על ביצועיו במשחקים בפועל (אירועי שערים, בישולים, הופעות וכרטיסים).
* **לוגיקה ומימוש:**
  * קליטת מזהה שחקן (`p_student_id INT`) ושליפת מחירו הנוכחי מטבלת `students`.
  * ספירת משחקים ייחודיים שבהם השתתף השחקן מתוך `match_lineups`.
  * מעבר בלולאת סמן (`FOR ... IN SELECT`) על אירועי השחקן ב-`match_events` וצבירת ניקוד: Goals (+5.0), Assists (+3.0), Matches (+1.0), Yellow Cards (-1.5), Red Cards (-4.0).
  * חישוב המחיר המותאם: $\text{New Price} = \text{Base Price} \times (1.0 + \frac{\text{Score}}{100.0})$.
  * עטיפה בבלוק `EXCEPTION` לטיפול במקרי קצה ובשגיאות קלט.
* **אימות ובדיקות:**
  * **בדיקה תקינה:** קריאה לשחקן קיים (`100000001`) מציגה חישוב שווי מדויק והדפסת הודעת `NOTICE` עם ערך שוק של `102.70`.
  * **בדיקה שלילית (Negative Test):** קריאה לשחקן שאינו קיים (`999999999`) נתפסת ב-`EXCEPTION` וזורקת חריגה מבוקרת.

![אימות פונקציה 1](images/f1_code.png) 

---

### פונקציה 2: בדיקת תאימות חוקי סגל ושווי כולל (`fn_evaluate_squad_compliance`)
* **קובץ מקור:** `step4/function2.sql` 
* **מטרה עסקית:** אימות עמידת סגל המשתמש בחוקי ליגת הפנטזי (15 שחקנים בסגל, 11 בהרכב הפותח, 4 בספסל, תקציב ושווי סגל מצטבר) והחזרת דוח סטטוס מובנה.
* **לוגיקה ומימוש:**
  * החזרת טיפוס מורכב ייעודי (`TYPE squad_compliance_result`) המכיל מזהה משתמש, כמויות שחקנים, שווי סגל, יתרת תקציב, סטטוס תאימות בוליאני והודעת הסבר.
  * בדיקת קיום המשתמש ב-`users` וזריקת שגיאה במידה ואינו קיים.
  * אגרגציה מ-`user_squads` ו-`students` לצורך חישוב שווי הסגל וספירת הרכב פותח וספסל.
  * בדיקה לוגית מרובת תנאים: אימות סך 15 שחקנים בסגל, אימות בדיוק 11 שחקני הרכב פותח, ובדיקת אי-חריגה מתקציב.
* **אימות ובדיקות:**
  * **אימות סגל קיים:** בדיקת משתמש `user_id = 1` זיהתה חוסר תאימות של 10 שחקנים פותחים במקום 11 (`Starting lineup must contain exactly 11 players (found 10)`), שווי סגל מחושב `1367.70`, וסטטוס `is_compliant = false`.
  * **בדיקה שלילית (Negative Test):** קריאה למשתמש לא קיים (`999999`) נתפסה בבלוק ה-`EXCEPTION` והחזירה הודעה מבוקרת.

![אימות פונקציה 2](images/f2_code.png) 

---

## 2. פרוצדורות (Stored Procedures)

### פרוצדורה 1: ניהול עסקאות והעברות שחקנים (`sp_process_player_transfer`)
* **קובץ מקור:** `step4/procedure1.sql` 
* **מטרה עסקית:** ניהול אטומי מלא של קנייה (`BUY`) ומכירה (`SELL`) של שחקן בשוק הפנטזי, כולל עדכון תקציב, עמלות שוק, עדכון סגל ותיעוד ביומן העסקאות.
* **לוגיקה ומימוש:**
  * קבלת פרמטרי קלט (`p_user_id`, `p_student_id`, `p_action`) ופרמטרי פלט (`INOUT p_success`, `INOUT p_message`).
  * נעילת שורת המשתמש (`FOR UPDATE`) למניעת Race Conditions.
  * **בעסקת BUY:** אימות שאין בעלות כפולה על השחקן, בדיקת יתרת תקציב, הוספת רשומה ל-`user_squads`, הפחתת תקציב המשתמש, ויצירת רשומה ב-`transactions`.
  * **בעסקת SELL:** אימות בעלות בסגל, חישוב עמלת מכירה (5%), זיכוי המשתמש ב-95% משווי השחקן, מחיקה מ-`user_squads`, ויצירת רשומה ב-`transactions`.
* **אימות ובדיקות:**
  * **קנייה תקינה:** משתמש 1 רכש את שחקן `100000005` בסך `97.10` (`p_success = true`).
  * **בדיקת כפילות:** ניסיון רכישה חוזר של אותו שחקן נחסם (`Player is already in the squad`).
  * **מכירה תקינה:** מכירת השחקן בעבור `92.25` (לאחר 5% עמלה), זיכוי התקציב ל-`303928.40`, ויצירת שורות טרנזקציה מתאימות ב-`transactions`.

![אימות פרוצדורה 1](images/p1_code.png) 

---

### פרוצדורה 2: יישוב וסגירת מחזור משחקים (`sp_settle_round`)
* **קובץ מקור:** `step4/procedure2.sql` 
* **מטרה עסקית:** סגירת מחזור משחקים פעיל, עדכון היסטוריית שווי השוק של כלל השחקנים בליגה, ופתיחת המחזור הבא.
* **לוגיקה ומימוש:**
  * בדיקה שהמחזור המבוקש קיים ונמצא בסטטוס `Active`. במידה וכבר הושלם (`Completed`) – זריקת שגיאה למניעת הרצה כפולה.
  * לולאת סמן העוברת על כל השחקנים בליגה, חישוב שווי מעודכן עבור כל אחד באמצעות `fn_calculate_player_market_value`, והזנת רשומה חדשה לטבלת `price_history`.
  * עדכון סטטוס המחזור הנוכחי ל-`Completed`.
  * איתור המחזור הבא בתור והפיכתו ל-`Active`.
* **אימות ובדיקות:**
  * **סגירת מחזור 503:** הפרוצדורה עיבדה בהצלחה **4,499 שחקנים** עם 0 שגיאות, עדכנה את `price_history`, והפעילה את מחזור 504.
  * **בדיקת נעילה:** ניסיון הרצה חוזר על מחזור 503 נחסם עם השגיאה: `Round 503 cannot be settled: current status is "Completed" (must be "Active")`.

![אימות פרוצדורה 2](images/p2_code.png) 

---

## 3. טריגרים (Triggers)

### טריגר 1: חישוב אוטומטי של מחיר שחקן בעת שינוי ציונים (`trg_students_price_autocalc`)
* **קובץ מקור:** `step4/trigger1.sql` 
* **פונקציית טריגר:** `trg_fn_students_price_autocalc()`
* **סוג טריגר:** `BEFORE INSERT OR UPDATE OF technical_rating, mental_rating ON students FOR EACH ROW`
* **מטרה עסקית:** שמירה על תמחור רציף ומדויק של שחקנים בבסיס הנתונים ללא צורך בהתערבות ידנית של מפתח האפליקציה.
* **לוגיקה:** שקלול דינמי: $\text{Price} = 10.0 + (\text{technical\_rating} \times 0.7) + (\text{mental\_rating} \times 0.6)$, והשמה ישירה ל-`NEW.current_price`.
* **מטרה עסקית:** קביעת מחיר בסיס ראשוני ואוטומטי לשחקן בעת יצירתו או שינוי נתוני הכושר/ציונים שלו (technical/mental), ללא צורך בהזנה ידנית. שווי השוק מביצועי משחקים מחושב בנפרד ע"י `fn_calculate_player_market_value`.
* **אימות ובדיקות:**
  * **בדיקת עדכון ציונים:** נתינת ערך התחלתי של ציוני השחקן ל-Technical: 95 ו-Mental: 90 ועל פי זה חישוב והצבה אוטומטית למחיר שיהיה 123.50 ב-`current_price`.

![אימות טריגר 1](images/t1_code.png) 

---

### טריגר 2: יומן ביקורת תקציב ומניעת יתרת חובה (`trg_users_budget_audit`)
* **קובץ מקור:** `step4/trigger2.sql` 
* **פונקציית טריגר:** `trg_fn_users_budget_audit()`
* **סוג טריגר:** `BEFORE UPDATE OF current_budget ON users FOR EACH ROW`
* **מטרה עסקית:** הגנה על שלמות נתוני התקציב ותיעוד מלא של שינויים כספיים לצרכי ביקורת (Audit Trail).
* **לוגיקה:**
  * בדיקה האם `NEW.current_budget < 0` – במידה וכן, חסימת הפעולה מיידית ע"י `RAISE EXCEPTION`.
  * אם הערך השתנה בצורה חוקית, הזנת רשומה לטבלת `user_budget_audit_log` עם הערך הקודם, החדש, סכום השינוי וחותמת זמן.
* **אימות ובדיקות:**
  * עדכון תקציב חוקי (הפחתה של 50.00): נוצרה שורת Audit תקינה עם `change_amount = -50.00`.
  * בדיקת שגיאה יזומה: ניסיון עדכון תקציב ל-`-100.00` נחסם ע"י המנוע עם השגיאה: `Update rejected: User budget cannot go negative (attempted -100.00)`.

![אימות טריגר 2](images/t2_code.png) 

---

## 4. תוכניות ראשיות (Main Programs)

### תוכנית ראשית 1: שקלול מחירי שחקנים וביצוע רכש אוטומטי (`Main1.sql`)
* **קובץ מקור:** `step4/Main1.sql` 
* **רכיבים משולבים:** פונקציה 1 (`fn_calculate_player_market_value`) + פרוצדורה 1 (`sp_process_player_transfer`).
* **מטרה ותרחיש עסקי:** סקירת שחקנים מועמדים שאינם בסגל המשתמש (באמצעות סמן מפורש), חישוב שווי השוק המותאם שלהם לפי ביצועים בעזרת `fn_calculate_player_market_value`, בדיקת יכולת מימון מול תקציב המשתמש בזמן אמת, וביצוע רכישה בפועל באמצעות `sp_process_player_transfer`.
* **בקרת זרימה ושגיאות:** הגבלת מכסת רכישות באמצעות `EXIT WHEN`, לולאות סמן, תנאי `IF/ELSE`, ובלוקי `EXCEPTION` מקוננים לבידוד שגיאות ברמת שחקן בודד (כולל אכיפת מכסה מרבית של 15 שחקנים).

![הרצת תוכנית ראשית 1](images/main1_execution.png)

---

### תוכנית ראשית 2: בקרת תאימות סגלים ויישוב מחזור ליגה (`Main2.sql`)
* **קובץ מקור:** `step4/Main2.sql` 
* **רכיבים משולבים:** פונקציה 2 (`fn_evaluate_squad_compliance`) + פרוצדורה 2 (`sp_settle_round`).
* **מטרה ותרחיש עסקי:** מעבר בלולאת סמן על כלל משתמשי הליגה הפעילים, הרצת בדיקת תאימות חוקי סגל ושווי בעזרת `fn_evaluate_squad_compliance`, צבירת נתונים סטטיסטיים כוללים (סך שווי סגלים בליגה, כמות סגלים תקינים מול לא-תקינים), ולאחר מכן איתור המחזור הפעיל וסגירתו באמצעות `sp_settle_round`.
* **בקרת זרימה ושגיאות:** לולאות עיבוד, צבירת מונים, טיפול בחריגות (`EXCEPTION`) ברמת משתמש וברמת תהליך הסגירה, ושאילתות אימות מפורטות בסיום.

![הרצת תוכנית ראשית 2](images/main2_execution.png)

---

## 5. טבלת סיכום רכיבי שלב ד'

| סוג הרכיב | שם האובייקט | קובץ | ייעוד ולוגיקה מרכזית | סטטוס בדיקה |
| :--- | :--- | :--- | :--- | :--- |
| **Function 1** | `fn_calculate_player_market_value` | `step4/function1.sql`  | שקלול ביצועים (אירועים/משחקים) לחישוב שווי שוק | עבר בהצלחה |
| **Function 2** | `fn_evaluate_squad_compliance` | `step4/function2.sql`  | בדיקת חוקיות הרכב פותח וספסל והחזרת טיפוס מורכב | עבר בהצלחה |
| **Procedure 1** | `sp_process_player_transfer` | `step4/procedure1.sql`  | ניהול מסחר אטומי (BUY/SELL), ניכוי עמלות ותיעוד | עבר בהצלחה |
| **Procedure 2** | `sp_settle_round` | `step4/procedure2.sql`  | יישוב מחזור המוני, הזנת היסטוריית שערים ומעבר שלב | עבר בהצלחה |
| **Trigger 1** | `trg_students_price_autocalc` | `step4/trigger1.sql`  | עדכון מחיר אוטומטי בשינוי ציוני שחקן ב-`students` | עבר בהצלחה |
| **Trigger 2** | `trg_users_budget_audit` | `step4/trigger2.sql`  | חסימת תקציב שלילי ותיעוד ב-`user_budget_audit_log` | עבר בהצלחה |
| **Main 1** | `Main Program 1 (DO block)` | `step4/Main1.sql`  | שילוב Function 1 + Procedure 1 לביצוע רכש אוטומטי מבוסס ביצועים | עבר בהצלחה |
| **Main 2** | `Main Program 2 (DO block)` | `step4/Main2.sql`  | שילוב Function 2 + Procedure 2 לבקרת סגלים ויישוב מחזור | עבר בהצלחה |


---

# שלב ה' – ממשק משתמש גרפי (Desktop GUI Application)

בשלב זה פותחה מערכת שולחנית (Desktop Application) מקיפה, מודרנית ואינטואיטיבית לניהול מלא של ליגת הכדורגל לבתי ספר וליגת הפנטזי (`School Football & Fantasy League Manager`). המערכת מתממשקת ישירות מול מסד הנתונים PostgreSQL ומנגישה את כלל היכולות שפותחו בשלבים הקודמים (טבלאות, שאילתות ופרוצדורות שמורות) תחת ממשק משתמש אלגנטי וידידותי.

---

## 1. כלי הפיתוח והטכנולוגיות (Tech Stack & Architecture)

המערכת פותחה תוך שימוש בסט כלים מתקדם בסביבת Python:

* **ספריית CustomTkinter:** ספריית GUI מתקדמת המבוססת על Tkinter ומספקת רכיבים גרפיים מודרניים עם תמיכה מלאה ומובנית בערכות נושא (Dark & Light Mode).
* **דרייבר psycopg2:** דרייבר חיבור חזק ויציב לעבודה ישירה, מרובת טרנזקציות וביצועים גבוהים מול מסד הנתונים PostgreSQL.
* **ספריית python-dotenv:** טעינה וניהול מאובטח של פרטי ההתחברות למסד הנתונים מתוך קובץ `.env`.
* **ספריית Faker:** העשרת נתוני כתובות ישראליות מציאותיות באנגלית עבור ישויות המערכת.
* **ארכיטקטורה מבוססת Multithreading (Async Dispatcher):** כלל השאילתות הכבדות וקריאות ה-DB מתבצעות ב-Worker Threads נפרדים בליווי תור משימות (`Queue`) ל-Main Thread, למניעת קיפאון של הממשק (UI Freezing).
* **מנגנון FkPicker מותאם ביצועים:** רכיב בחירה דינמי עם חיפוש חכם המגביל את פריטי התפריט ל-80 תוצאות בו-זמנית, למניעת קריסות מערכת וצריכת זיכרון מול טבלאות ענק (מעל 25,000 שורות).

---

## 2. הוראות התקנה והפעלה (Setup & Run Instructions)

### שלב א': הכנת סביבת העבודה והתקנת תלויות
יש לוודא שמותקנת סביבת Python 3.10 ומעלה, ולפתוח את הטרמינל בתיקיית הפרויקט:

    pip install -r step5/requirements.txt

### שלב ב': הגדרת קובץ התחברות (`.env`)
יש לוודא שקיים קובץ `.env` תקין בתיקיית הפרויקט עם פרטי הגישה ל-PostgreSQL:
...
### שלב ג': הפעלת האפליקציה
להרצת הממשק הגרפי, יש להריץ את הפקודה:

    python step5/main.py

---

## 3. תיאור מסכי המערכת וצילומי מסך

### מסך ראשי / דשבורד כניסה (Dashboard & Main Navigation)
מסך הכניסה של המערכת מרכז מבט-על (KPIs) על מצב הליגה והנתונים ומאפשר ניווט מלא ומהיר לכל אחד מחלקי המערכת דרך תפריט צד מודרני.

![Dashboard Overview](images/gui_dashboard.png)

---

### ניהול נתונים (Manage Data - CRUD Operations)
ממשק לניהול מלא (הוספה, עריכה, מחיקה וחיפוש) של כל 18 טבלאות המערכת. הממשק מסווג לפי קטגוריות ראשיות בתפריט הצד (Core School Data, Equipment & Facilities, Matches & Activities, Fantasy League & Trading):
* **הסתרת מפתחות טכניים:** המשתמש אינו נחשף למזהי ID פנימיים, והם מנוהלים אוטומטית ברקע.
* **בחירה היררכית חכמה:** בחירת שחקנים וקבוצות נעשית תוך סינון לפי בית ספר למניעת עומס נתונים.
* **טיפול בשגיאות שלמות:** מניעת מחיקת רשומות מקושרות (Foreign Key Constraints) בליווי דיאלוג שגיאה ידידותי ומפורט.

![Manage Data Screen](images/gui_crud_view.png)
---

### דוחות ותובנות (Reports & Insights)
מסך המנגיש את השאילתות העסקיות המורכבות (משלב ב'). המשתמש יכול לבחור דוח מתוך תפריט, להזין פרמטרים דינמיים (כמו כמות שערים מינימלית או תאריכים), ולקבל טבלת תוצאות מעובדת ומסודרת.

![Reports and Insights](images/gui_reports_view.png)

---

### פעולות ותהליכים עסקיים – Actions & Routines
ממשק ייעודי להרצת הפרוצדורות והפונקציות השמורות (משלב ג') באמצעות טאבים ממוקדים:
1. **חישוב שווי שוק (Calculate Market Value):** חישוב ועדכון שווי שוק עדכני של שחקן בהתאם לביצועיו.
2. **בדיקת חוקיות סגל (Evaluate Squad Rules):** אימות חוקיות ועמידה במגבלות התקציב והסגל של מנהל פנטזי.
3. **שוק העברות (Transfer Market):** ביצוע טרנזקציית קנייה/מכירה של שחקן בין מנהלי פנטזי תוך עדכון תקציבים.
4. **סגירת מחזור ליגה (Settle League Round):** סגירת מחזור משחקים, שמירת היסטוריית מחירי שחקנים וקידום הליגה.

כל פעולה מציגה בסיומה באנר סטטוס צבעוני וידידותי המפרט את תוצאות הביצוע ללא הצגת מזהים טכניים.

![Actions & Operations](images/gui_actions_view.png)
